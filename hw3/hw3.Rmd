...
On one hand, if the true curve has degree higher than 3, $\hat{g_1}$ will not be able to capture it at all, while $\hat{g_2}$ can capture it. So $\hat{g_2}$ will have the smaller test error in this case.
...
$$
\text{ozone}^{\frac{1}{3}}=0.002\times \text{radiation} + 0.045\times \text{temperature}-0.071\times\text{wind}+0.077
$$
This is not a very satisfactory model as the $R^2 = 0.63$.

```{r, echo=F}
ozone <- read.table("C:/Users/xingw/Desktop/503/stats503/hw3/ozone_data.txt", header=1)
train <- sample(nrow(ozone), floor(nrow(ozone)*0.7))
ozone$cbr <- ozone$ozone^(1/3)
lmod <- lm(cbr~., data=ozone[train, ])
summary(lmod)
```
...
for(i in 1:nrow(train_loo)){
  ...
  for(k in 1:10){
    ...
    #q <- q[2:(length(q)-1)]
    gam_mod <- gam(cbr~ns(radiation, k)+ns(temperature, k)+ns(wind, k), data=train1)
...
plot(pred-ozone[-train, 'cbr'], ylab=expression(Ozone^(1/3)), main='linear model residual plot')
...
plot(gam_pred-ozone[-train, 'cbr'], ylab=expression(Ozone^(1/3)), main='GAM residual plot')
